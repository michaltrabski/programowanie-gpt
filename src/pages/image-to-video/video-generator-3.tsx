...
async function imageToVideo({ image, durationSec, textOpts, audioBlob }: ImageToVideoInput): Promise<Blob> {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      // MediaRecorder for the initial silent video
      ...
      mediaRecorder.onstop = async () => {
        ...
        // If audio needed combine, else resolve
        if (audioBlob) {
          try {
            const muxed = await combineVideoAndAudioWebm(webmBlob, audioBlob, durationSec);
            resolve(muxed);
          } catch (err: any) {
            resolve(webmBlob); // fallback (just video)
          }
        } else {
          resolve(webmBlob);
        }
      ...
      // Render loop (slowed to correct fps)
      let currentFrame = 0;
      function drawLoop() {
...
// -- Hack: combine video and mp3 by re-recording them played together in browser
async function combineVideoAndAudioWebm(videoBlob: Blob, audioBlob: Blob, duration: number): Promise<Blob> {
  return new Promise((resolve, reject) => {
    ...
    // Offscreen canvas for drawing video
    const canvas = document.createElement("canvas");
    canvas.width = VIDEO_WIDTH;
    canvas.height = VIDEO_HEIGHT;
    const ctx = canvas.getContext("2d")!;

    // Draw video frames (realtime)
    const FPS = 30;
    let frame = 0;
    function draw() {
      try {
    ...
    // Combine audio MP3 as a MediaStream
