...
// --- NEW Helper: Data Sanitizer ---
// Ensures all arrays and nested objects exist to prevent .map() crashes
const sanitizeCVData = (parsed: any): CVData => {
  // 1. Merge with defaults to ensure top-level keys exist
  const safeData = { ...INITIAL_DATA, ...parsed };

  // 2. Ensure Top-Level Arrays exist
  ...
  // 3. Deep Sanitize: Ensure Nested Arrays exist
  ...
  // 4. Handle edge case where summary might be a string (from bad LLM output)
        ...
        <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-red-50 border border-red-200 rounded-xl text-red-600">
          ...
          <button
            onClick={() => {
              ...
              // Optional: You could trigger a global reset here if needed
            }}
  ...
  useEffect(() => {
    if (jsonString && jsonString !== "undefined") {
      ...
      try {
        ...
        // Only set error for Syntax issues.
        // Structural issues are fixed by sanitizeCVData silently.
...
// --- NEW COMPONENT: Prompt Modal (Updated) ---
const PromptModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const [userCV, setUserCV] = useState(() => {
    if (typeof window !== "undefined") {
      return localStorage.getItem("prompt_userCV") || "";
    }
    return "";
  });
  ...
  useEffect(() => {
    localStorage.setItem("prompt_userCV", userCV);
  }, [userCV]);

  if (!isOpen) return null;

  // 3. Construct the Final Prompt (Dynamic)
  const finalPrompt = `You are working on your CV to apply for a job.

Prepare information to your CV in object json using example-cv-data-format.

Focus the most on summary - About me section.
summary: 
    {
      id: "1",
      main: - this shoud math the requirement from JOB DESCRIPTION
      text: this shoul be an explanation why I match the requirement from JOB DESCRIPTION
    }

Ouput just json data.
Output json data must match exactly format from example-cv-data-format.
Data output should be in the same language like open-role-information

--- JOB DESCRIPTION ---
<open-role-information>
${jobDescription}
</open-role-information>

--- MY CURRENT CV INFORMATION ---
<your-personal-information>
${userCV}
</your-personal-information>

Create information for my CV to apply for a job.
Output information in json format.
Don't use any information from example json.
I will copy paste json to my application that will ouptut json as visualized vc.
Output just json without any other text or information.
...
<example-cv-data-format>
INITIAL_DATA must be of type CVData
</example-cv-data-format>
`;
      ...
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-4 border-b flex justify-between items-center bg-gray-50">
          <h2 className="font-bold text-gray-800 flex items-center gap-2">
            <span className="text-xl">✨</span> Generate ChatGPT Prompt
          </h2>
          <button
            onClick={onClose}
        ...
        <div className="p-6 overflow-y-auto flex-1 grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col h-[45%]">
              <label className="block text-xs font-bold text-gray-500 uppercase mb-2">
                2. Paste Your Current CV Info
              </label>
              <textarea
                className="flex-1 w-full p-3 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none resize-none"
                placeholder="Paste your old CV text, LinkedIn bio, or JSON data here..."
                value={userCV}
          ...
          <div className="flex flex-col h-full">
            <label className="block text-xs font-bold text-purple-600 uppercase mb-2">
              3. Final Prompt <br />
              (Copy text and paste into ChatGPT, Gemini, or similar tools!)
            </label>
            <div className="relative flex-1">
              <textarea
                readOnly
                className="w-full h-full p-4 text-xs font-mono text-gray-700 bg-gray-100 border border-gray-200 rounded-lg focus:outline-none resize-none"
                value={finalPrompt}
              />
              {/* Copy Overlay Button (Floating inside) */}
              <button
                onClick={handleCopy}
                disabled={!jobDescription.trim() || !userCV.trim()}
                className={`absolute bottom-4 right-4 px-4 py-2 text-sm font-bold text-white rounded-lg shadow-lg transition-all flex items-center gap-2 ${
                  copied
            ...
            <p className="text-[10px] text-gray-400 mt-2 text-center">
              Paste the result above into ChatGPT, Gemini, or Claude to get your tailored CV.
            </p>
          </div>
        </div>
