...
const CVGenerator: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-[#f3f4f6] p-6 font-sans">
      ...
      {/* --- RENDER PROMPT MODAL --- */}
      <PromptModal isOpen={isPromptModalOpen} onClose={() => setIsPromptModalOpen(false)} data={data} />
    </div>
  );
};
...
// --- NEW COMPONENT: Prompt Modal ---
const PromptModal = ({ isOpen, onClose, data }: { isOpen: boolean; onClose: () => void; data: CVData }) => {
  const [jobDescription, setJobDescription] = useState("");
  const [copied, setCopied] = useState(false);
  const cvDataString = JSON.stringify(data, null, 2);

  if (!isOpen) return null;

  const generatePrompt = () => {
    return `I am applying for the following job role. Please rewrite my CV data (specifically the summary, experience descriptions, and skills) to better align with this job description while maintaining truthfulness. Keep the output in the exact same JSON format provided.

--- JOB DESCRIPTION ---
${jobDescription}

--- MY CURRENT CV DATA (JSON) ---
${cvDataString}
`;
  };
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 print:hidden">
      <div className="bg-white rounded-xl shadow-2xl w-full max-w-2xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1">
          ...
          <div>
            <label className="block text-xs font-bold text-gray-500 uppercase mb-2">
              2. Your Current CV Data (JSON)
            </label>
            <textarea
              readOnly
              className="w-full h-32 p-3 text-[10px] font-mono text-gray-600 bg-gray-100 border border-gray-200 rounded-lg focus:outline-none resize-none"
              value={cvDataString}
            />
          </div>
        </div>
        ...
        <div className="p-4 border-t bg-gray-50 flex justify-end gap-3">
          ...
          <button
            ...
            className={`px-4 py-2 text-sm font-bold text-white rounded-lg transition-all flex items-center gap-2 ${
              copied ? "bg-green-600" : "bg-black hover:bg-gray-800 disabled:opacity-50 disabled:cursor-not-allowed"
            }`}
          >
            {copied ? (
              <>
