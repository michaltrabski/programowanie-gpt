...
// =========================================
// NEW HELPER: Editable Time Input
// =========================================
...
const TimeInput: React.FC<TimeInputProps> = ({ value, onCommit }) => {
  const [localValue, setLocalValue] = useState(value.toFixed(2));

  // Sync local state when the prop changes (e.g. via +/- buttons)
  useEffect(() => {
  ...
  const handleBlur = () => {
    ...
    onCommit(num);
    setLocalValue(num.toFixed(2)); // Reformat nicely
  };
  ...
  return (
    <input
      className="w-16 bg-black text-center text-white font-mono p-1 rounded border border-gray-600 focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
      value={localValue}
...
const SegmentEditor: React.FC<SegmentEditorProps> = ({ segments, onUpdate, onPreview }) => {
  // Unified update function for both buttons and text input
  const updateSegmentTime = (index: number, position: 0 | 1, exactValue: number) => {
    const updated = [...segments];
    // Ensure value is non-negative and formatted properly
    updated[index][position] = Math.max(0, Number(exactValue.toFixed(2)));
    onUpdate(updated);

    // --- LOGIC: Smart Preview ---
    const [segStart, segEnd] = updated[index];
    let previewRange: TimeRange;

    if (position === 0) {
      // If editing START: Play the WHOLE segment
      previewRange = [segStart, segEnd];
    } else {
      // If editing END: Play only the LAST 0.5 seconds (check cut-out)
      const pStart = Math.max(segStart, segEnd - 0.5);
      previewRange = [pStart, segEnd];
    }
        ...
        {segments.map((seg, idx) => (
          ...
          >
            <div className="flex items-center gap-3 w-full xl:w-auto justify-between xl:justify-start">
              ...
              {/* Play Button: Plays FULL segment */}
              <button
