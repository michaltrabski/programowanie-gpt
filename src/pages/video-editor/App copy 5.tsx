...
const App: React.FC = () => {
  ...
  // State to track if we need to auto-calculate duration from metadata
  ...
  const loadSettingsForFile = useCallback((fileName: string) => {
    ...
    // No save found: Set flag to true
  ...
  // =========================================
  // 5. METADATA & DURATION HANDLING (FIXED)
  // =========================================

  // Shared helper to update segments based on duration
  const applyDurationToSegments = useCallback((duration: number) => {
    if (!isNaN(duration) && duration !== Infinity && duration > 0) {
      ...
      setShouldAutoSetDuration(false); // Done
      console.log(`Auto-set default segment: 0 - ${duration}`);
    }
  }, []);

  // 1. Event Listener version (Standard)
  ...
  // 2. Race Condition Fix: Check if metadata is ALREADY loaded when the flag turns true
  useEffect(() => {
    if (shouldAutoSetDuration && player1Ref.current) {
      // readyState >= 1 means metadata is loaded
      if (player1Ref.current.readyState >= 1) {
        console.log("Metadata already loaded, applying immediately...");
  ...
  const playSegmentStep = useCallback(
    (segmentIndex: number, currentPlayerIdx: 0 | 1, currentSegments: TimeRange[]) => {
      ...
      // Ensure volume is on (if you want it muted by default, remove this)
    ...
    <div className="min-h-screen bg-gray-900 text-white flex flex-col items-center p-8 font-sans">
      ...
      <div className="relative w-full max-w-2xl bg-black aspect-video rounded-lg overflow-hidden border border-gray-800 mb-6">
        <video
          ...
          preload="auto"
          muted // Helps with autoplay policies, though we unmute on play
