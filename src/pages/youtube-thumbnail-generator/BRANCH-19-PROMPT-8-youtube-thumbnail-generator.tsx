        ...
        <ColorPalette
          ...
          onChange={(idxOrColor) => {
            if (typeof idxOrColor === "number") {
              ...
              // If an external color (color picker), just set to custom at the end (display only)
      ...
      >
        <div
          ...
          style={{
            ...
            left: 0,
            top: 0,
            // === FIX: Proper opacity on preview (background alpha only for bg, not for text!) ===
            ...
            fontFamily: "Inter, Arial, sans-serif",
            minWidth: 80,
            opacity: 1, // keep text fully opaque, only the bg uses alpha!
...
const OverlayImageEditor: React.FC = () => {
  ...
  // --------- FIX: Download: match preview exactly for multiline text, font, bg, opacity ---------
  const handleDownload = useCallback(() => {
    ...
    img.onload = () => {
      ...
      // -------- BACKGROUND --------
      // proper alpha blending with background only
      ...
      // Render each line with proper vertical centering and identical spacing
      lines.forEach((line, i) => {
        const totalHeight = overlayState.mainFontSize * lines.length;
        // Compute Y to vertically center, matching the CSS preview
