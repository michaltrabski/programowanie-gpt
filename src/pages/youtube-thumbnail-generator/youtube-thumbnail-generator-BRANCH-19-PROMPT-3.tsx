...
import Draggable, { type DraggableData, type DraggableEvent } from "react-draggable"; // << NEW
...
type OverlayBoxProps = {
  ...
  radius: number;
  position: { x: number; y: number }; // normalized [0,1] - this will be mapped to pixel location
  previewW: number;
  previewH: number;
  onPosChange: (pos: { x: number; y: number }) => void; // << ADDED
};
...
const OverlayBox: React.FC<OverlayBoxProps> = ({
  ...
  // Estimate width in px dynamically (for min/max clamp)
  let estWidth = 200;
  ...
  const x = position.x * previewW - textWidth / 2;
  const y = position.y * previewH - textHeight / 2;
  ...
  // ReactDraggable expects position as px-offset from top/left
  ...
  function handleDrag(e: DraggableEvent, data: DraggableData): void {
    ...
    // Clamp within 0..1, but not allowing even half the overlay box out of the viewport
