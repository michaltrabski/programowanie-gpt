...
const ARROW_MIN_SCALE = 0.15; // not exposed in ui, but for future scaling
...
export type OverlayState = {
  ...
  // In the future you might support arrow.scale, arrow.angle, etc.
};
...
function loadOverlayState(): OverlayState {
  try {
    ...
    // Basic sanitize, arrowPos fallback
    return {
      ...initialOverlayState,
      ...parsed,
...
// --- Draggable Arrow using react-moveable ---
...
const DraggableArrow: React.FC<DraggableArrowProps> = ({ arrowImg, position, previewW, previewH, onPosChange }) => {
  ...
  // Size of arrow
  const arrowW = DEFAULT_ARROW_SIZE.w * (previewW / IMAGE_WIDTH);
  const arrowH = DEFAULT_ARROW_SIZE.h * (previewH / IMAGE_HEIGHT);

  // Compute current left/top in px (centered)
  const left = position.x * previewW - arrowW / 2;
  const top = position.y * previewH - arrowH / 2;

  // Clamp arrow so it stays in the visible preview
  const getClampedNorm = (x_px: number, y_px: number) => {
    const minNormX = arrowW / 2 / previewW;
    const maxNormX = 1 - minNormX;
    const minNormY = arrowH / 2 / previewH;
    const maxNormY = 1 - minNormY;
    return {
      x: Math.max(minNormX, Math.min(maxNormX, x_px / previewW)),
      y: Math.max(minNormY, Math.min(maxNormY, y_px / previewH)),
    };
  };

  // Moveable requires target to exist before mounted, so defer until DOM
  useEffect(() => {
    // react-moveable fixes pointer-events by itself, but keep ref consistent
  }, []);
    ...
    <>
        ...
        {arrowImg ? (
          ...
          <div
            style={{
              ...
              fontSize: Math.floor(Math.max(14, arrowW / 6)),
              color: "#c44",
            }}
          >
            Arrow...
      ...
      {arrowImg && (
        <Moveable
          ...
          container={arrowRef.current?.parentElement || undefined}
          origin={false}
          draggable={true}
          throttleDrag={0}
          edge={false}
          ...
          // Controls styling
          props={{
            className: "custom-moveable-arrow",
          }}
          dragArea={false}
          bounds={{
            left: 0,
            top: 0,
            right: previewW - arrowW,
            bottom: previewH - arrowH,
          }}
          onDrag={({ left: newLeft, top: newTop }) => {
            // Clamp "center" of arrow to [0,1]
            const cx = (newLeft ?? 0) + arrowW / 2;
            const cy = (newTop ?? 0) + arrowH / 2;
            const norm = getClampedNorm(cx, cy);
            onPosChange(norm);
          }}
          // actual position tied to <div> left/top, so changing overlayState.arrowPos updates it immediately
          snappable={true}
          snapThreshold={5}
        />
      )}
...
const OverlayImageEditor: React.FC = () => {
  ...
  // Load arrow PNG from ARROW_PNG_URL, which is always available
  useEffect(() => {
  ...
  // Download: match preview, overlay, and arrow PNG exactly
  const handleDownload = useCallback(() => {
    ...
    img.onload = () => {
      ...
      // Arrow's position and scaling matches preview:
    ...
    >
      ...
      >
        ...
        >
          ...
          <li>Drag the arrow wherever you want!</li>
