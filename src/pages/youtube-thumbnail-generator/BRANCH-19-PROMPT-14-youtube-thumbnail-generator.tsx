    ...
    return (
      <>
        ...
        <Moveable
          ...
          onDrag={(e: OnDrag) => {
            // Use absolute left/top from Moveable to avoid drift when dimensions update
            const nextLeft = Math.max(0, Math.min(previewW - widthPx, e.left));
            const nextTop = Math.max(0, Math.min(previewH - heightPx, e.top));

            onPosChange({
              x: (nextLeft + widthPx / 2) / previewW,
              y: (nextTop + heightPx / 2) / previewH,
            });
          }}
          onResize={(e: OnResize) => {
          ...
          <button
            ...
            onClick={() =>
              setBox((s) => ({
                ...
                widthN: Math.max(minTextWidthPx, 80) / Math.max(1, previewW),
                heightN: Math.max(minTextHeightPx, 40) / Math.max(1, previewH),
              }))
            }
          >
            Reset rotation/size
      ...
      {arrowImg && (
        <>
          <Moveable
            ...
            onDrag={(e: OnDrag) => {
              // Use absolute e.left/e.top instead of deltas to prevent drift
              const newLeft = Math.max(0, Math.min(previewW - widthPx, e.left));
              const newTop = Math.max(0, Math.min(previewH - heightPx, e.top));
    ...
    >
      ...
      >
        ...
        >
          ...
          <li>
            Click "Download" to export PNG (preview and PNG will both show the overlay text and arrow in your chosen
            positions)
          </li>
        </ul>
      </div>
