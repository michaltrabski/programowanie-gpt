...
import Moveable, { type OnDrag, type OnResize, type OnRotate, type OnScale, type OnWarp } from "react-moveable";
...
// --------- Utility functions ----------
// ... colorWithAlpha, drawRoundRect, toHex6, loadOverlayState, saveOverlayState (unchanged, keep from your code)
  ...
  <div style={{ display: "flex", flexDirection: "column", ...style }}>
    ...
    <div style={{ display: "flex", gap: 5 }}>
      ...
      {allowCustom && (
        <input
          type="color"
          value={typeof selected === "string" ? toHex6(selected) : palette[selected as number] || "#ffffff"}
...
const OverlayBox = forwardRef<HTMLDivElement, OverlayBoxProps>(
  (
    ...
    const [estDims, setEstDims] = useState<{ width: number; lines: number }>({ width: 200, lines: 1 });
    useEffect(() => {
    ...
    // Init moveable x/y from normalized center pos
    useEffect(() => {
      setState((s) => ({
        ...s,
        // keep rotation/scale/skew!
        x: position.x * previewW - textWidth / 2,
        y: position.y * previewH - textHeight / 2,
        width: textWidth,
        height: textHeight,
      }));
      // eslint-disable-next-line
    }, [position.x, position.y, textWidth, textHeight, previewW, previewH]);
    ...
    return (
      <>
        <div
          ...
          style={{
            ...
            transform: `
              ...
              skew(${state.skew[0]}deg, ${state.skew[1]}deg)
            `,
            // don't allow miniscule box (if scaled too much)
            minWidth: 80,
          }}
          aria-label="Overlay Main Text (drag/rotate/scale/resize/skew)"
          title="Drag/resize/rotate/scale/skew overlay text"
        >
        ...
        <Moveable
          ...
          rotatable
          scalable
          pinchable={["rotatable", "resizable", "scalable"]}
          keepRatio={false}
          ...
          onDrag={({ left, top }: OnDrag) => {
            ...
            // update center in normalized space
            onPosChange({
              x: (left + state.width / 2) / previewW,
              y: (top + state.height / 2) / previewH,
            });
          }}
          onResize={({ width, height, drag }: OnResize) => {
            ...
            // update overlaystate center on drag
            onPosChange({
              x: (drag.left + width / 2) / previewW,
              y: (drag.top + height / 2) / previewH,
            });
          }}
          onRotate={({ beforeRotate }: OnRotate) => {
            setState((s) => ({ ...s, rotation: beforeRotate }));
          }}
          onScale={({ scale }: OnScale) => {
            setState((s) => ({ ...s, scale }));
          }}
          onWarp={({ matrix }: OnWarp) => {
            // simulate skew (keep only matrix 3,1 and 1,3 as skew in deg)
            // Not a perfect mapping, for demo
            const skewX = Math.atan(matrix[2]) * (180 / Math.PI);
            const skewY = Math.atan(matrix[1]) * (180 / Math.PI);
            setState((s) => ({ ...s, skew: [skewX, skewY] }));
          }}
          throttleResize={0}
          throttleRotate={0}
          origin={false}
        ...
        <div style={{ fontSize: 10, opacity: 0.7, userSelect: "none" }}>
          <button
            ...
            onClick={() => setState((s) => ({ ...s, rotation: 0, scale: [1, 1], skew: [0, 0] }))}
          >
            Reset rotation/scale/skew
          </button>
        </div>
      </>
    );
...
const DraggableArrow: React.FC<DraggableArrowProps> = ({ arrowImg, position, previewW, previewH, onPosChange }) => {
  const arrowRef = useRef<HTMLDivElement>(null);
  // moveable state: x, y, rotation, scale
  const [state, setState] = useState({
    x: 0,
    y: 0,
    width: DEFAULT_ARROW_SIZE.w * (previewW / IMAGE_WIDTH),
    height: DEFAULT_ARROW_SIZE.h * (previewH / IMAGE_HEIGHT),
    rotation: 0,
    scale: [1, 1] as [number, number],
    skew: [0, 0] as [number, number],
  });

  // adjust moveable x/y from normalized center pos & base size
  useEffect(() => {
    const w = DEFAULT_ARROW_SIZE.w * (previewW / IMAGE_WIDTH) * state.scale[0];
    const h = DEFAULT_ARROW_SIZE.h * (previewH / IMAGE_HEIGHT) * state.scale[1];
    setState((s) => ({
      ...s,
      width: w,
      height: h,
      x: position.x * previewW - w / 2,
      y: position.y * previewH - h / 2,
    }));
    // eslint-disable-next-line
  }, [position.x, position.y, previewW, previewH]);

  // Helper for clamping center position
  const getClampedNorm = (left: number, top: number, width: number, height: number) => {
    const minNormX = width / 2 / previewW;
    const maxNormX = 1 - minNormX;
    const minNormY = height / 2 / previewH;
    const maxNormY = 1 - minNormY;
    return {
      x: Math.max(minNormX, Math.min(maxNormX, (left + width / 2) / previewW)),
      y: Math.max(minNormY, Math.min(maxNormY, (top + height / 2) / previewH)),
    };
  };
        ...
        {arrowImg ? (
          <img
            ...
            style={{
              ...
              userSelect: "none",
              opacity: 0.95,
              filter: "drop-shadow(0 2px 12px rgba(206,54,33,0.19)) drop-shadow(0 1px 5px rgba(80,0,0,0.12))",
            }}
          />
        ) : (
          <div
            style={{
              ...
              fontSize: Math.floor(Math.max(14, state.width / 6)),
              color: "#c44",
            }}
          >
            Arrow...
      ...
      {arrowImg && (
        <>
          <Moveable
            ...
            onDrag={({ left, top }) => {
              setState((s) => ({ ...s, x: left, y: top }));
              // update parent with new normalized center
              onPosChange(getClampedNorm(left, top, state.width, state.height));
            }}
            onResize={({ width, height, drag }) => {
              ...
              onPosChange(getClampedNorm(drag.left, drag.top, width, height));
            }}
            ...
            rotationPosition="top"
          />
          <div style={{ fontSize: 10, opacity: 0.7, userSelect: "none", pointerEvents: "auto" }}>
            <button
              style={{
                fontSize: 11,
                cursor: "pointer",
            ...
            >
              Reset rotation/scale/skew/size
            </button>
          </div>
        </>
      )}
...
const OverlayImageEditor: React.FC = () => {
  ...
  // The Render/Download code is unchanged
  ...
  const handleDownload = useCallback(() => {
    ...
    img.onload = () => {
      ...
      // main text box, MATCH preview & OverlayBox dx
      ctx.save();
      ...
      ctx.fillStyle = bgPalette[typeof overlayState.mainBgIdx === "number" ? overlayState.mainBgIdx : 0] || "#222";
      ...
      lines.forEach((line, i) => {
        ...
        const y = posY - totalHeight / 2 + overlayState.mainFontSize * i + overlayState.mainFontSize / 2;
        ctx.fillText(line.length ? line : " ", posX, y);
      });
      ctx.restore();
      ctx.restore();

      // -------- ARROW PNG (draw after box/text so it's always top-most) --------
    ...
    >
      ...
      >
        ...
        >
          ...
          <li>Drag, rotate, scale, skew, or resize the overlay text and arrow!</li>
          <li>All settings persist via localStorage</li>
          <li>
            Click "Download" to export PNG (preview and PNG will both show the overlay text <b>and</b> arrow in your
            chosen positions)
          </li>
        </ul>
      </div>
