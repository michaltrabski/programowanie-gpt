        ...
        <div style={{ display: "flex", gap: 2, alignItems: "center" }}>
          {TEXT_COLORS.map((c, i) => (
            <ColorButton
              key={`main-txtc-${i}`}
              color={c}
              selected={toHex6(state.mainTextColor) === toHex6(c)}
          ...
          <input
            ...
            onChange={(e) =>
              setState({
                ...state,
                mainTextColor: toHex6(e.target.value),
              })
            }
            style={{
              width: 28,
...
const OverlayBox = forwardRef<HTMLDivElement, OverlayBoxProps>(
  (
    ...
    ref // actual dom ref, passed from parent (not needed here, but makes composable)
  ) => {
