...
function colorWithAlpha(color: string, alpha: number): string {
  // Accepts CSS color (named, hex, rgb(a)) and produces "rgba(...)"
    ...
    <div style={{ display: "flex", gap: 5 }}>
      ...
      {allowCustom && (
        <input
          ...
          onChange={(e) => {
            if (isBg) {
              onChange(e.target.value); // Custom BG (sets at index -1 effect)
            } else {
...
const DraggableArrow: React.FC<DraggableArrowProps> = ({ arrowImg, position, previewW, previewH, onPosChange }) => {
  ...
  const handleDrag = (_e: DraggableEvent, data: DraggableData): void => {
    const center = {
      x: (data.x + arrowW / 2) / previewW,
      y: (data.y + arrowH / 2) / previewH,
    };
    // Clamp so arrow never leaves visible area entirely
  ...
  return (
    <Draggable bounds="parent" position={controlledPosition} onDrag={handleDrag} onStop={handleDrag} nodeRef={arrowRef}>
      <div
        ref={arrowRef}
        style={{
          position: "absolute",
...
const OverlayImageEditor: React.FC = () => {
  ...
  // FIX: Download: match preview, overlay, and arrow PNG exactly
