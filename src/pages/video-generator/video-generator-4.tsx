  ...
  try {
    if (audioBlob) {
      ...
      const audioBuf = await new Promise<AudioBuffer>((resolve, reject) => {
        // Some browsers still use callback-based decodeAudioData
      ...
      // Not connecting to audioCtx.destination prevents audible playback; uncomment to monitor:
      ...
      if (tracks[0]) {
        ...
        // If we couldn't make an audio track, continue with silent video
    ...
    try {
      ...
      img.onload = async () => {
        try {
          ...
          // Ensure AudioContext autoplay policies satisfied: we started rendering from a user click
          setProcessing(false);
        } catch (err: any) {
