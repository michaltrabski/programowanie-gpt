...
// Draws image in the center of canvas keeping aspect ratio, with blue bars as needed.
...
function createZip(files: { blob: Blob; name: string }[]): Promise<Blob> {
  // Very minimal zip encoder for two files.
  // For max compatibility, recommend using JSZip in a real project.
  // Here, for only 2 files, let's use minimal-zip (from MIT: https://github.com/cthackers/adm-zip/blob/master/zip.js)
  // But for brevity and to avoid extra dependencies, here is a minimal implementation:
  // This requires extra dependency for real use; so here, fallback: if no JSZip, just download individually.
  return import("jszip")
    .then((JSZipModule) => {
...
const ImageToVideoGenerator: React.FC = () => {
  ...
  async function handleDownloadBoth() {
    ...
    try {
      ...
      // Fallback: show per-file download links
      alert("Could not create ZIP (JSZip not loaded), please download the videos one by one.");
    } finally {
  ...
  const VideoPreview = videoResults.length === 2 && (
    <div
      style={{
        display: "flex",
        gap: 16,
    ...
    >
      {["horizontal", "vertical"].map((orientation) => {
        ...
        const aspect = orientation === "horizontal" ? 1920 / 1080 : 1080 / 1920;
        return (
          <div key={orientation} style={{ textAlign: "center", flex: "1 1 0" }}>
            <div style={{ fontSize: 15, fontWeight: 600, marginBottom: 4 }}>
              {orientation === "horizontal" ? "Horizontal (1920x1080)" : "Vertical (1080x1920)"}
            </div>
            <video
              src={v.url}
              controls
    ...
    >
      ...
      {videoResults.length === 2 && (
        <div style={{ marginBottom: 22 }}>
          ...
          <div style={{ fontSize: 13, color: "#999", marginTop: 5, textAlign: "center" }}>
