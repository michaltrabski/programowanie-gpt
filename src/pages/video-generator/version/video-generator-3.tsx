...
const ImageToVideoGenerator: React.FC = () => {
  ...
  // Tries to get both, if available
  ...
  async function handleGenerateVideo() {
    ...
    try {
      recorder = new window.MediaRecorder(destStream, { mimeType: "video/webm" });
    } catch (err) {
      ...
      >
        {imageUrl ? (
          <img
            src={imageUrl}
            alt="Preview"
            style={{ maxWidth: 240, maxHeight: 180, display: "block", margin: "0 auto 8px" }}
          />
        ) : (
