...
async function loadFfmpegCDN() {
  // Dynamically load @ffmpeg/ffmpeg from CDN, singleton
  // @ts-ignore
  if (window.FFmpeg && window.createFFmpeg) return window.createFFmpeg;
  ...
  // Wait a moment for global
  let waited = 0;
  for (;;) {
    // @ts-ignore
    if (window.createFFmpeg) break;
...
const ImageToVideoGenerator: React.FC = () => {
  ...
  async function generateSingleVideo(
    ...
    // Compose with audio – for now, **NO audio** for MediaRecorder (we will mux in ffmpeg)
  ...
  // ffmpeg-wasm helper: mux video and mp3 together via ffmpeg command
  async function muxWithMp3(inputVideoBlob: Blob, mp3Blob: Blob, expectedDuration?: number) {
    ...
    // Clamp duration to shorter of video/mp3 if both available
    let ffmpegCmd = [
      "-i", vidName,
      "-i", mp3Name,
      "-map", "0:v:0",
    ...
    if (expectedDuration && !isNaN(expectedDuration) && expectedDuration > 1) {
      // Clamp output duration (if needed)
      ffmpegCmd = [
        "-i", vidName,
        "-i", mp3Name,
        "-t", expectedDuration + "",
  ...
  async function handleGenerateVideo() {
    ...
    try {
      ...
      const recBlob = await generateSingleVideo(
        ...
        undefined /* do not try to mix audio in MediaRecorder */
      );

      let outBlob: Blob = recBlob;
      let outType = "video/webm";
      let outExt = "webm";
      // If user supplied MP3, use ffmpeg WASM to mux
      if (mp3File) {
        setFfStatus("Preparing mp3 and video...");
        let mp3Blob = mp3File;
        if (mp3Blob.type !== "audio/mp3" && mp3Blob.type !== "audio/mpeg") {
          // Convert to correct mp3 via ffmpeg if really needed, but skip for now
        }
        ...
        {videoResults.map((v) => (
          <div key={v.orientation} style={{ textAlign: "center", flex: "1 1 0" }}>
            ...
            <div>
              <a
                href={v.url}
                download={`video-gen-${v.orientation}.mp4`} // It's always mp4 now
                style={{
