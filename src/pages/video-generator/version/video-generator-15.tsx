...
type TextSettings = {
  ...
  fontFamily: string;
  fontWeight: string;
  fontSizePercent: number; // % of video width
  fontMinPx: number;
  fontMaxPx: number;
  textColor: string;
  bgColor: string;
...
const DEFAULT_TEXT_SETTINGS: TextSettings = {
  ...
  fontWeight: "bold",
  fontSizePercent: 7,    // % of video width (reasonably scales up/down!)
  fontMinPx: 24,
  fontMaxPx: 80,
  textColor: "#222",
  bgColor: "#eeeeee",
  boxOpacity: 0.90,
  outlineColor: "#000",
  outlineWidth: 0, // px, try 2 or 3 for outline effect
  align: "center",
  boxRadius: 18,
};
...
function drawImageFullWidth(
  ...
  if (
    ...
    const {
      overlayText, fontFamily, fontWeight, fontSizePercent, fontMinPx, fontMaxPx,
      textColor, bgColor, boxOpacity, outlineColor, outlineWidth, align, boxRadius,
    } = opts.settings;
    // Draw at bottom, max width = 50% video
    const padY = 18;
    const maxTextWidth = canvasWidth * 0.5;

    // --- Font size AUTO SCALED based on VIDEO WIDTH (& min/max limits) ---
...
const ImageToVideoGenerator: React.FC = () => {
  ...
  // Load/restore settings from localStorage
  useEffect(() => {
    try {
  ...
  // Save settings into localStorage live
  useEffect(() => {
    try {
  ...
  // ----------- Video Generation: includes text overlay -------------
  async function generateSingleVideo(
    ...
    drawImageFullWidth(ctx, width, height, img, { settings: textSettings, showText: false });
    ...
    const recorderOptions: MediaRecorderOptions = mimeType
      ? { mimeType, audioBitsPerSecond: 128_000, videoBitsPerSecond: 2_500_000 }
      : { audioBitsPerSecond: 128_000, videoBitsPerSecond: 2_500_000 };
    ...
    videoResults.length === 2 && (
      ...
      >
        {(["horizontal", "vertical"] as Orientation[]).map((orientation) => {
          ...
          return (
            <div key={orientation} style={{ textAlign: "center", flex: "1 1 0" }}>
      ...
      <div style={{ marginTop: 10, padding: 10, background: "#f6fbfe", borderRadius: 6 }}>
        <div style={{ marginBottom: 10 }}>
          ...
          <div style={{ color: "#777", fontSize: 12, marginTop: 4 }}>
            The above text will appear centered at the bottom, over a box, after 0.5s.
          </div>
        </div>
