...
type TextSettings = {
  ...
  positionYPercent: number; // used if position=custom (from top, 0=top, 100=bottom)
};
...
const ImageToVideoGenerator: React.FC = () => {
  ...
  async function generateSingleVideo(
    ...
    function tick() {
      ...
      if (inputType === "video" && visual instanceof HTMLVideoElement) {
        // Static seek: we emulate video playback by stepping video.currentTime.
        if (
          ...
          try {
            ...
            // advance currentTime for next frame
            const nextT = Math.min(
              visual.duration,
              vidCurrent / FPS // keep in sync!
            );
    ...
    return await new Promise<VideoResult>(async (resolve, reject) => {
      ...
      if (inputType === "video" && visual instanceof HTMLVideoElement) {
        // On loaded, begin advancing frames by time
        vidCurrent = 0;
        visual.currentTime = 0;
        // On loadedmetadata, play to first frame
        // we don't play() because want accurate seeking
        let frames = Math.ceil(finalDuration * FPS);
        videoTick = null;

        function onSeeked() {
          // nothing, we manage CT below
        }

        visual.addEventListener("seeked", onSeeked);

        setTimeout(async () => {
          // time to stop recorder
          try {
  ...
  async function handleGenerateVideo() {
    ...
    try {
      ...
      const resH = await generateSingleVideo(
        loadedMedia,
        "horizontal",
        inputType,
        videoDuration,
      ...
      const resV = await generateSingleVideo(
        loadedMedia,
        "vertical",
        inputType,
        videoDuration,
  ...
  async function handleDownloadBoth() {
    ...
    try {
      const files = videoResults.map((v) => ({
        blob: v.blob,
        name: `video-gen-${v.orientation}.webm`,
      }));
          ...
          return (
            <div key={orientation} style={{ textAlign: "center", flex: "1 1 0" }}>
              ...
              <div>
                <a
                  href={v.url}
                  download={`video-gen-${orientation}.webm`}
                  style={{
