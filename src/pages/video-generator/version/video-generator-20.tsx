...
const ImageToVideoGenerator: React.FC = () => {
  ...
  function handleDrop(e: DragEvent<HTMLDivElement>) {
    ...
    if (file) {
      ...
      setVideoResults([]);
      // keep any selected mp3 (so user can add mp3 to video). Do not reset mp3.
    }
  }
  ...
  function handleSelectVisual(e: ChangeEvent<HTMLInputElement>) {
    ...
    if (file) {
      ...
      setVisualUrl(URL.createObjectURL(file));
      setVideoResults([]);
      // keep mp3 selection to allow adding mp3 to video
    }
    e.target.value = "";
  }
    ...
    if (mp3) {
      ...
      getMp3Duration(mp3U)
        .then((dur) => {
          ...
          // Only override duration when we are generating from an image
          if (visualType !== "video") {
            setDuration(dur);
          }
        })
  ...
  // Helper to pick a good bitrate for quality
  function pickBitrates(width: number, height: number) {
    // For 1080p use ~12–16 Mbps, scale roughly with pixels
    const pixels = width * height;
    const base = 12_000_000; // 12 Mbps for ~1080p
    const factor = Math.max(1, pixels / (1920 * 1080));
    const videoBitsPerSecond = Math.floor(base * factor);
    const audioBitsPerSecond = 256_000; // 256 kbps for good MP3/AAC-like quality
  ...
  async function generateSingleVideo(
    ...
    // Audio routing rules:
    // - If video input:
    //    - If MP3 present: mix video audio WITH mp3 (adds mp3 to video)
    //    - Else: use original video audio
    // - If image input:
    //    - If MP3 present: use MP3
    //    - Else: silent
    const wantVideoAudio = inputType === "video";
    const wantMp3Audio = !!inputMp3Url;
    if (wantVideoAudio || wantMp3Audio) {
      audioContext = new (window.AudioContext || (window as any).webkitAudioContext)();

      const dest = audioContext.createMediaStreamDestination();

      // Connect video audio if available
      if (wantVideoAudio && visual instanceof HTMLVideoElement) {
        const videoSource = audioContext.createMediaElementSource(visual);
        const videoGain = audioContext.createGain();
        videoGain.gain.value = wantMp3Audio ? 0.7 : 1.0; // reduce if mixing
        videoSource.connect(videoGain).connect(dest);
        // Avoid local speaker playback from the <video> element
        try {
          visual.muted = true;
          visual.volume = 0;
        } catch {}
      ...
      // Connect MP3 if requested
      if (wantMp3Audio && inputMp3Url) {
        audioObj = new Audio();
        audioObj.src = inputMp3Url;
        audioObj.crossOrigin = "anonymous";
        audioObj.preload = "auto";
        audioObj.loop = inputType === "video"; // loop to cover full video if video is longer than mp3
        // Do not play to device speakers
        audioObj.muted = true;
        audioObj.volume = 0;

        const mp3Source = audioContext.createMediaElementSource(audioObj);
        const mp3Gain = audioContext.createGain();
        mp3Gain.gain.value = wantVideoAudio ? 0.7 : 1.0;
        mp3Source.connect(mp3Gain).connect(dest);
      }
    ...
    // Recorder options tuned for quality
    ...
    function startVideoLoop(videoEl: HTMLVideoElement) {
      ...
      if (hasRVFC) {
        const onVideoFrame = () => {
          ...
          // manual requestFrame to keep encoder busy
          try {
            (canvasStream.getVideoTracks()[0] as any)?.requestFrame?.();
          } catch {}
        };
    ...
    // Fix: Ensure both video and audio start in sync
    return await new Promise<VideoResult>(async (resolve, reject) => {
      ...
      try {
        recorder.start(); // let it buffer internally
      } catch {
      ...
      if (inputType === "video" && visual instanceof HTMLVideoElement) {
        ...
        // Start mp3 audioObj if present
        let mp3Started = false;
        if (audioObj) {
          const startMp3 = async () => {
            if (mp3Started) return;
            mp3Started = true;
            try {
              await audioObj.play();
            } catch {
          ...
          // Ensure MP3 is loaded
          if (audioObj.readyState >= 2) {
            startMp3();
          } else {
            audioObj.addEventListener("canplay", startMp3, { once: true });
          }
        }

        // Start video loop
        // Wait for video to be able to play
        ...
        // Play both (video/muted) and mp3 in tight sync
        visual.play().catch(() => {});
        if (audioObj && !mp3Started) {
          audioObj.play().catch(() => {});
        }

        startVideoLoop(visual);

        // Stop the recorder when video ends
        const onEnded = () => {
          ...
          }, 100); // Slight delay to avoid race with audio sync issues
        };
        visual.addEventListener("ended", onEnded);

        // Safety: stop at (duration+0.5)s just in case
        ...
        if (audioObj) {
          // Audio: play, stop recorder at end
          const onEnded = () => {
          ...
          // If MP3 is much longer, still put a safety timer just in case
          ...
          // No audio => just stop after N seconds
          startImageLoop();
          stopTimeoutId = setTimeout(() => {
            try {
              if (recorder.state !== "inactive") recorder.stop();
            ...
            <div style={{ marginLeft: 10 }}>
              <label>
                ...
                <input
                  ...
                  onChange={(e) => setSetting("positionYPercent", Number(e.target.value) || 0)}
          ...
          >
            ...
            {visualIsVideo ? (
              <div style={{ marginTop: 6, color: "#777" }}>
                MP3 will be added to the video audio (mixed). Leave MP3 empty to keep only the original video audio.
              </div>
            ) : (
              <div style={{ marginTop: 6, color: "#777" }}>
