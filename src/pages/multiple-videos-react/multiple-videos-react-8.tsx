...
const MultiVideoPlayer: React.FC = () => {
  ...
  const progress = Number((persisted.progress || 0).toFixed(0));
  ...
  // On progress change, sync time for all videos, value rounded to one decimal
  useEffect(() => {
    const rounded = Number(progress.toFixed(0));
    videoRefs.forEach((ref) => {
      if (!ref.current || !Number.isFinite(rounded)) return;
      try {
        ref.current.currentTime = Math.min(rounded, ref.current.duration || rounded);
      } catch {}
    });
  }, [progress, videoRefs]);
  ...
  // Sync progress timer for navigation (drive all videos) — round to 1 decimal
  useEffect(() => {
    function tick() {
      const curVid = videoRefs[mainVideoIdx]?.current;
      if (!curVid) return;
      setPersisted((p) => ({ ...p, progress: Number(curVid.currentTime.toFixed(0)) }));
  ...
  function handleSliderChange(e: ChangeEvent<HTMLInputElement>) {
    ...
    setPersisted((p) => ({ ...p, progress: Number(value.toFixed(0)) }));
  }
