...
// === UPDATED: Use your given video URLs and names (full path) ===
...
const MultiVideoPlayer: React.FC = () => {
  // One ref for each video element
  const videoRefs = useMemo(() => VIDEO_URLS.map(() => React.createRef<HTMLVideoElement>()), []);
  // Master preview video refs (one per video, stacked, we show only the "main" one)
  ...
  // Get global min duration as all videos load
  useEffect(() => {
  ...
  // Sync time on all videos (all video player refs and master preview)
  useEffect(() => {
    // Video refs for clickable videos
  ...
  // On slider/seek, switch active video if needed
  useEffect(() => {
  ...
  // Change main video at current time
  ...
  // Use RAF to keep progress synced (use master preview visible ref as clock source)
  useEffect(() => {
    function tick() {
  ...
  // When master preview video is played/paused, update state
  useEffect(() => {
    function handlePlay() {
      setPlaying(true);
    }
