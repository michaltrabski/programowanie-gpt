...
const MultiVideoPlayer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    function tick() {
      ...
      if (!ref) return;
      const currentTimeFloor = Math.floor(ref.currentTime);
      console.log("tick()", currentTimeFloor);
      // setPersisted((p) => ({ ...p, progress: currentTimeFloor }));
    ...
    // eslint-disable-next-line
  }, [playing, mainVideoIdx, masterPreviewRefs]);
