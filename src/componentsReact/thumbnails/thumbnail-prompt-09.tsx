...
const OverlayImageEditor: React.FC = () => {
  ...
  // --------- Helper for overlay render on PREVIEW (NOT input)
  // Adjusts visual font size so it matches downloaded image
  ...
  >) => {
    ...
    // Estimate text width for centering background box
    // More precise would require measuring, but it's good enough for mono fonts & single lines
    // We could use canvas.measureText here if we want total precision.
    // But for now let's use a 'dummy' span element to measure.
    ...
    return (
      <div
        ...
        onPointerDown={(e: React.PointerEvent<HTMLDivElement>) => {
          ...
          e.preventDefault();
          e.stopPropagation();
          // get offset of pointer within the box
  ...
  // --------- Input section generator (OUTSIDE preview)
    ...
    >
      ...
      <input
        ...
        onChange={(e) => {
          ...
          if (!fontSizes.includes(v)) {
            // Allow any value in allowed range for number input (not just preset list)
            v = Math.max(Math.min(v, Math.max(...fontSizes)), Math.min(...fontSizes));
          }
          setFontSize(v);
        }}
      />
    ...
    >
      ...
      {image && (
        <div style={{ width: "100%", maxWidth: 1100 }}>
          {/* --- All inputs and font size above the image --- */}
