...
const OverlayImageEditor: React.FC = () => {
  ...
  const [mainRadius, setMainRadius] = useState<number>(28);
  const [secRadius, setSecRadius] = useState<number>(21);

  // Arrow overlay controls
  const [showArrow, setShowArrow] = useState<boolean>(false);
  const [arrowPos, setArrowPos] = useState<{ x: number; y: number }>({ x: 0.9, y: 0.2 });
  const [arrowAngle, setArrowAngle] = useState<number>(-45); // in degrees
  const [arrowColor, setArrowColor] = useState<string>("#FFF");
  const [arrowSize, setArrowSize] = useState<number>(220);
  ...
  useEffect(() => {
    function onMove(e: PointerEvent) {
      ...
      // For arrow, allow a bit outside. For text overlays limit so it can't disappear.
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      // --- ARROW
      if (showArrow) {
        // Arrow as drawn in preview: sizes are in screen pixels, so remap
  ...
  function renderArrowOverlay({
    ...
    return (
      ...
      >
        ...
        >
          <g stroke={color} strokeWidth={size / 13} strokeLinecap="round" fill="none">
        ...
        <div style={{ width: "100%", maxWidth: 2000 }}>
          ...
          <details style={{ marginBottom: 11, marginTop: 5, textAlign: "center" }} open={showArrow}>
            ...
            >
              ...
              />
              <span style={{ color: "#aaa", fontSize: 16 }}>Drag arrow on image to move</span>
            </div>
          </details>
