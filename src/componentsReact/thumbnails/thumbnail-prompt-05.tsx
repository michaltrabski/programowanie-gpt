...
// --- Utility: Fast palette extractor from an image element
...
const OverlayImageEditor: React.FC = () => {
  ...
  // When palette is loaded, prioritize differing bg colors
  useEffect(() => {
    setMainBgIdx(0);
    setSecBgIdx(1);
  }, [palette.length]);
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      if (mainText.trim()) {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(mainText, posX, posY + 5);
        ctx.restore();
      }
      // --- Secondary text
      if (secondaryText.trim()) {
        ...
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(secondaryText, posX, posY + th / 16);
        ctx.restore();
      }
  ...
  // Only for preview area (i.e. when image loaded)
  // Mouse events are on the overlays only for dragging
  const overlayRefs = {
  ...
  const renderOverlayBox = ({
    ...
    // For drag
    const dragRef = overlayRefs[boxKey as "main" | "sec"];
    const dragStart = (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      setDraggingBox(boxKey as "main" | "sec");
    };
      ...
      <>
        ...
        <div
          ...
          style={{
            ...
            userSelect: "none",
            cursor: draggingBox === (boxKey as "main" | "sec") ? "grabbing" : "grab",
            MozUserSelect: "none",
            WebkitUserSelect: "none",
            pointerEvents: "auto",
            zIndex: 10,
            outline: draggingBox === (boxKey as "main" | "sec") ? "2.5px dashed #66f" : undefined,
            border: draggingBox === (boxKey as "main" | "sec") ? "1px solid #66f" : undefined,
            opacity: draggingBox && draggingBox !== (boxKey as "main" | "sec") ? 0.6 : 1,
        ...
        {/* The textbox input (not draggable) sits absolutely at bottom */}
        <input
          type="text"
