...
const PRESET_COLORS = [
  "#F44242", "#F58D2D", "#F8D94F", "#43d964", "#5183f9"
];

const TEXT_COLORS = [
  "#FFF", "#222", "#E3E3E3", "#000", "#ffd700", "#00ffbe", "#ef40ea", "#ff3800", "#22aaff"
];
const ARROW_COLORS = [
  "#FFF", "#222", "#F44242", "#43d964", "#5183f9", "#ffda00", "#ff3800", "#ef40ea"
];
...
const OverlayImageEditor: React.FC = () => {
  ...
  // Overlay positions
  const [mainPos, setMainPos] = useState<{ x: number; y: number }>(
    persisted?.mainPos ?? { x: 0.5, y: 0.5 }
  );
  ...
  const [arrowPos, setArrowPos] = useState<{ x: number; y: number }>(
    persisted?.arrowPos ?? { x: 0.9, y: 0.2 }
  );
  ...
  useEffect(() => {
    savePersistedState({
      ...
      mainBgIdx, secBgIdx,
      mainFontSize, secFontSize,
      mainAlpha, secAlpha,
      mainTextColor, secTextColor,
      mainRadius, secRadius,
      mainPos, secondaryPos,
      showArrow, arrowPos, arrowAngle, arrowColor, arrowSize
    });
  }, [
    mainText, secondaryText,
    mainBgIdx, secBgIdx,
  ...
  useEffect(() => {
    if (!image) { setPalette(PRESET_COLORS); return; }
  ...
  useEffect(() => {
    function onMove(e: PointerEvent) {
      ...
      if (draggingResize && showArrow) {
        ...
        if (!arrowDiv) return;
        const aw = arrowDiv.offsetWidth, ah = arrowDiv.offsetHeight;
        const ax = arrowPos.x * bounds.width, ay = arrowPos.y * bounds.height;
        ...
        const ARROW_MIN_SIZE = 30, ARROW_MAX_SIZE = 600;
  ...
  const handleDownload = () => {
    ...
    img.onload = () => {
      ...
      if (ir > wr) { sh = ih; sw = sh * wr; sx = (iw - sw) / 2; sy = 0; }
      else { sw = iw; sh = sw / wr; sx = 0; sy = (ih - sh) / 2; }
      ...
      // --- main text ---
      {
        ...
        const paddingX = 54, paddingY = 26, rad = mainRadius;
      ...
      // --- secondary text ---
      {
        ...
        const metrics = ctx.measureText(displayText);
        const paddingX = 24, paddingY = 13, rad = secRadius;
        const textWidth = metrics.width + paddingX * 2;
  ...
  const ColorButton: React.FC<{
    ...
    <div
      ...
      style={{
        ...
        cursor: "pointer", outline: "none", marginLeft: 1, marginRight: 1
      }}
    ...
    return (
      <div
        ...
        style={{
          ...
          background: palette[bgIdx]
            ? colorWithAlpha(palette[bgIdx], bgAlpha)
            : "rgba(34,34,34,0.79)",
  ...
  function renderArrowOverlay({
    pos, angle, color, size
  }: {
    pos: { x: number, y: number }, angle: number, color: string, size: number
  }) {
    const previewW = previewRef.current?.clientWidth ?? IMAGE_WIDTH;
    const previewH = previewRef.current?.clientHeight ?? IMAGE_HEIGHT;
        ...
        <div
          ...
          onPointerDown={(e: React.PointerEvent<HTMLDivElement>) => {
            ...
            if (handleDom) {
              ...
              if (
                e.clientX >= handleBounds.left && e.clientX <= handleBounds.right &&
                e.clientY >= handleBounds.top && e.clientY <= handleBounds.bottom
              ) return;
            }
            e.preventDefault(); e.stopPropagation();
        ...
        >
          <svg width={size*1.03} height={size/5} viewBox={`0 0 ${size} ${size/5}`}
            ...
            <g stroke={color} strokeWidth={arrowThickness} strokeLinecap="round" fill="none">
              ...
              <polyline
                points={`${size*0.87},${size/10} ${size*0.76},${size/19} ${size*0.87},${size/10} ${size*0.76},${(size*0.19)}`}
              />
            </g>
          </svg>
        </div>
  ...
  function renderOverlayControls({
    ...
    fontSize: number, setFontSize: (fs: number) => void, fontSizes: number[],
    palette: string[], bgIdx: number, setBgIdx: (i: number) => void,
    radius: number, setRadius: (r: number) => void,
    placeholder: string, ariaLabel: string, inputMaxLength: number,
    showPalette: boolean, boxKey: "main" | "sec",
    alpha: number, setAlpha: (a: number) => void,
    textColor: string, setTextColor: (c: string) => void
  }) {
    return (
      <div style={{
        display: 'flex', alignItems: 'center', gap: 17, marginBottom: 12, flexWrap: 'wrap',
        ...
        <input
          ...
          step={1}
          style={{
            width: 58, fontSize: 16, padding: "6px 5px",
            borderRadius: 8, border: "none", background: "#1b1c34", color: "#fff",
            textAlign: "center", boxShadow: "0 1.5px 6px #0002"
          }}
          onChange={e => {
        ...
        <div style={{ display: "flex", gap: 2, alignItems: "center", marginRight: 3}}>
          ...
          <input
            ...
            style={{
              width: 31, height: 31, borderRadius: 8, border: "none", outline: "none", padding: 0, background: "none"
            }}
          />
        </div>
        ...
        <input
          ...
          step={1}
          style={{
            width: 48, fontSize: 16, padding: "6px 6px",
            borderRadius: 8, border: "none", background: "#202033", color: "#fff",
            textAlign: "center"
          }}
          onChange={e => {
            let v = Number(e.target.value);
        ...
        />
        <span style={{ fontSize: 15, width: 43, color: "#ddd", display:"inline-block" }}>{Math.round(alpha*100)}%</span>
      </div>
    );
  }
      ...
      {image && (
        <div style={{ width: "100%", maxWidth: 2000 }}>
          ...
          <details style={{marginBottom: 11, marginTop: 5, textAlign:"center"}} open={showArrow}>
            ...
            <div style={{
              display:"flex", flexWrap:"wrap", gap:17, alignItems: "center", justifyContent: "center", padding:12
            }}>
              ...
              <input
                ...
                step={1}
                style={{
                  width: 55, fontSize: 16, padding: "6px 4px",
                  borderRadius: 8, border: "none", background: "#202033", color: "#fff",
                  textAlign: "center"
                }}
              ...
              <input
                ...
                step={1}
                style={{
                  width: 48, fontSize: 16, padding: "6px 5px",
                  borderRadius: 8, border: "none", background: "#202033", color: "#fff",
                  textAlign: "center"
                }}
                title="Arrow length"
          ...
          >
            ...
            {showArrow && renderArrowOverlay({
              pos: arrowPos, angle: arrowAngle, color: arrowColor, size: arrowSize
            })}
          </div>
          {/* Buttons */}
